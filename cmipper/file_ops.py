...
def get_min_max_coords_from_xa_d(
    ...
    lat_coord = next(
        (coord for coord in lat_coord_possibilities if coord in xa_d.coords), None
    )
    lon_coord = next(
        (coord for coord in lon_coord_possibilities if coord in xa_d.coords), None
    )
    ...
    return (
        xa_d[lat_coord].values.min(),
        xa_d[lat_coord].values.max(),
        xa_d[lon_coord].values.min(),
        xa_d[lon_coord].values.max(),
    )
...
def rename_nc_with_coords(
    ...
    min_lat, max_lat, min_lon, max_lon = get_min_max_coords_from_xa_d(
        nc_xa, lat_coord_name, lon_coord_name
    )

    lats_strs, lons_strs = formatted_lat_lon_from_vals(
...
def extract_lat_lon_ranges_from_fp(fp: Path | str):
    ...
    coord_pattern = re.compile(
        r".*_[ns](?P<north>-?\d+-?.?\d?)_[ns](?P<south>-?\d+-?.?\d?)_"
        r"[we](?P<west>-?\d+-?.?\d?)_[we](?P<east>-?\d+-?\.?\d*).*.nc",
        re.IGNORECASE,
    )
    ...
    if not coord_match:
        raise ValueError(
            f"Could not extract latitudes and longitudes from {fp}. Ensure the filename is formatted correctly."
        )

    # Extract cardinal directions and determine signs
    cardinal_directions = re.findall(
        r"([nsew])(?=-?\d+)", str(Path(fp).name), re.IGNORECASE
    )
...
def find_intersecting_cmip(
    ...
    cmip6_dir_fp = (
        config.cmip6_data_dir / source_id / member_id / "regridded"
        if not cmip6_data_dir
        else Path(cmip6_data_dir)
    )
    ...
    correct_fps = return_files_spanning_date_range(
        correct_area_fps, year_range=sorted(year_range)
    )
